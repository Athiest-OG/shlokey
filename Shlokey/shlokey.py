...
pali_to_eng_dict = {v.lower(): k for k, v in pali_dict.items()}
brahmi_to_pali_dict = {v: k for k, v in brahmi_dict.items()}

# English to Pali conversion
def english_to_pali(text):
    words = text.lower().split()
    pali_translation = [pali_dict.get(word, f"[{word}]") for word in words]  # Mark untranslated words
    return " ".join(pali_translation)

# Pali to Brahmi conversion
def pali_to_brahmi(text):
    brahmi_translation = "".join([brahmi_dict.get(char, f"[{char}]") for char in text])  # Mark untranslated chars
    return brahmi_translation

# Brahmi to Pali conversion
def brahmi_to_pali(text):
    pali_translation = "".join([brahmi_to_pali_dict.get(char, f"[{char}]") for char in text])
    return pali_translation

# Pali to English conversion
def pali_to_english(text):
    words = text.split()
    english_translation = [pali_to_eng_dict.get(word.lower(), f"[{word}]") for word in words]
